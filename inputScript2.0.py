...
cycChoice = 'test' #can be ratio or loop or test
solveRat = 1e-5
...
#ptDStart, ptDIncrement, ptDEnd for the direction the load will be applied. 0 is x and to the left, 90 is y into the page, etc
#ptDList = list(np.arange(ptDStart, ptDEnd + ptDIncrement, ptDIncrement))
...
my_experiment = gen.experiment(filePath, functionPath, outFileName, iterator, cycChoice, functionHandles, movieHandles, plots, solveRatio, loadTypes,
                               eqVert = eqVertVal, eqFw = eqFwVal, eqD = eqDList, eqS = eqSList, numCycLoops = 1, numCycles = 100, arraySize = 3000000)

# define joint materials
mortar_stone = gen.jointMaterial({'jkn':5.0e7, 'jks':5.0e7, 'jfric': 30})
