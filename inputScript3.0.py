...
#INPUT SCRIPT
# set up experiment
filePath= 'C:/Users/Rebecca Napolitano/Documents/datafiles/labWalls/diffusionMaps/current/eqonly/'  
functionPath = 'C:\\Users\\Rebecca Napolitano\\Documents\\GitHub\\generate3DEC\\'
outFileName = '2018_6_14_additionalEQ__3DEC'
...
saveCycValue = 'no' #can be a yes or no string and turns on if cycstates are saved. good normally, bad for diffusion maps bc memory
...
# define materials(dens, edge, fixity, hide, ymod, join) !!!! YOU NEED EDGE AND YOUNGS IF YOU ARE DOING EQ
#mortar = material({'dens':2200, 'edge':100, 'hide':True})
fixedstone = gen.material({'edge':0.5, 'dens':2500,'bulk': 2e9, 'shear':1e9, 'fixity':'fix',})
brick = gen.material({'edge':0.5, 'dens':2508.67, 'bulk': 2e9, 'shear':1e9})

#______________________________________________________________

#define loads
loadTypes = ['eq'] #can be 'eq','pt', or 'none' right now
...
eqDIncrement = np.pi/16
eqDEnd = np.pi/2
eqDList = []
eqDList = list(np.arange(eqDStart, eqDEnd + eqDIncrement*0.5, eqDIncrement))
...
eqSEnd = 0.3
eqSList = []
eqSList = list(np.arange(eqSStart, eqSEnd + eqSIncrement*0.5, eqSIncrement)) #cant calculate eqBoundLoad outhere since it will change with each eqS
...
ptVEnd = 0.0
ptVList = []
ptVList = list(np.arange(ptVStart, ptVEnd + ptVIncrement*0.5, ptVIncrement))
#print(ptVList)
...
ptLList = [["none"]]#'y', 'range z 0.33 0.6']]
...
my_experiment = gen.experiment(filePath, functionPath, outFileName, iterator, cycChoice, functionHandles, movieHandles, plots, solveRatio, loadTypes,
                               eqFreq = eqFreqVal, eqD = eqDList, eqS = eqSList, 
                               ptV = ptVList, ptL = ptLList, saveCyc = saveCycValue,
                               numCycLoops = 1, numCycles = 100, arraySize = 3000000)

# define joint materials
mortar_stone = gen.jointMaterial({'jkn':8e7, 'jks':5e7, 'jfric': 30})
